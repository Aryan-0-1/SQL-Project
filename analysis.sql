...
SELECT 
	p.product_name, SUM(s.sales_amount) as total_revenue FROM sales s
LEFT JOIN products p
...
SELECT 
	c.customer_key, c.first_name, c.last_name, SUM(s.sales_amount) as total_revenue FROM sales s
LEFT JOIN customers c
...
SELECT 
	c.first_name, c.last_name, COUNT(DISTINCT(s.order_number)) as total_orders FROM sales s
LEFT JOIN customers c
    ON s.customer_key = c.customer_key
GROUP BY s.customer_key, c.first_name, c.last_name
...
SELECT
    ...
    WHEN (current_sales - (AVG(current_sales) OVER(PARTITION BY product_name))) < 0 THEN 'Below Avg' ELSE 'Avg'
 	END AS avg_change,
    ...
    WHEN current_sales - (LAG(current_sales) OVER(PARTITION BY product_name ORDER BY order_year)) < 0 THEN 'Decrease' 	ELSE 'No Change'
	END AS py_change
...
SELECT cost_range, COUNT(product_name) AS total_products FROM product_segment
GROUP BY cost_range
ORDER BY total_products DESC
